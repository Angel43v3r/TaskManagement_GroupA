...
export const checkIssueModify = async (req, res, next) => {
    try {
        ...
        if (!req.params.id) {
            ...
            return res.status(403).json({ message: `Access denied: only developers and admins can create new issues. Your role: '${user.role}'`});
        }
        ...
        return res.status(500).json({ message: 'Server error checking modify permission' });
    }
};
...
export const checkIssueDelete = async (req, res, next) => {
    try {
        ...
        return res.status(403).jsoon({ message: 'Access Denied: only the admin or reporter of this issue can delete it.' });
    } catch (err) {
        console.error('Delete permission error: ', err);
        return res.status(500).json({ message: 'Server error checking delete permission' });
    }
};